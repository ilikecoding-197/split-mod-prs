...
IDE_Morph.prototype.setDefaultTheme = function () {
  // dark
  IDE_Morph.prototype.isBright = false;
...
IDE_Morph.prototype.scriptsTexture = function () {
  var scale = SyntaxElementMorph.prototype.scale,
    pic = newCanvas(new Point(100 * scale, 100 * scale)), // bigger scales faster
...
IDE_Morph.prototype.openIn = function (world) {
  ...
  function autoRun() {
    ...
    if (isLoadingAssets()) {
      myself
        .world()
        .animations.push(new Animation(nop, nop, 0, 200, nop, autoRun));
    } else {
      myself.runScripts();
    }
  }
  ...
  function interpretUrlAnchors() {
    ...
    if (location.hash.substr(0, 6) === "#open:") {
            ...
            pic.onload = (async () => {
              let buff = new Uint8Array(await blob.arrayBuffer()),
                ...
                hasImportanbleCode = (txt) =>
                  txt.match(/^<(blocks|block|script|sprite)/i),
                embeddedData,
                canvas;
      ...
      if (hash.substr(0, 8) === "<project") {
        ...
        applyFlags(
          myself.cloud.parseDict(hash.substr(hash.indexOf("</project>") + 10))
        );
...
IDE_Morph.prototype.createProjectControlBar = function () {
  ...
  var padding = 5,
    ...
    myself = this;
  if (this.projectControlBar) {
    this.projectControlBar.destroy();
  }
  ...
  button = new ToggleButtonMorph(
    ...
    "toggleStageSize",
    [new SymbolMorph("smallStage", 16), new SymbolMorph("normalStage", 16)],
  ...
  this.projectControlBar.add(stageSizeButton);
  this.projectControlBar.stageSizeButton = button; // for refreshing
  ...
  this.projectControlBar.add(appModeButton);
  this.projectControlBar.appModeButton = appModeButton; // for refreshing
  ...
  button.labelShadowColor = colors[1];
  if (true) {
    button.labelColor = new Color(236, 89, 89);
  } else {
  ...
  button.refresh();
  stopButton = button;
  this.projectControlBar.add(stopButton);
  this.projectControlBar.stopButton = stopButton; // for refreshing
  ...
  button.refresh();
  pauseButton = button;
  this.projectControlBar.add(pauseButton);
  ...
  this.projectControlBar.refreshResumeSymbol = function () {
    var pauseSymbols;
    if (
...
IDE_Morph.prototype.createControlBar = function () {
  ...
  this.controlBar.updateLabel = function () {
    ...
    scene =
      myself.scenes.at(1) !== myself.scene
        ? " (" + myself.scene.name + ")"
        : "";
...
IDE_Morph.prototype.createCategories = function () {
  var myself = this,
    categorySelectionAction = this.scene.unifiedPalette
      ? scrollToCategory
      : changePalette,
  ...
  function addCategoryButton(category, color) {
    ...
    var labelWidth = 10,
      colors = [
        ...
        color?.darker?.() ||
          SpriteMorph.prototype.blockColor[category].darker(),
    ...
    button.label.setPosition(
      new Point(
        button.bounds.origin.x +
          button.width() / 2 -
          (button.label.right() - button.label.bounds.origin.x) / 2 +
          2,
        24
      )
    );
    ...
    if (color) {
      button.userMenu = () => {
        ...
        menu.addItem("Remove Category", () =>
          myself.deletePaletteCategory(category)
        );
  ...
  this.palette.reactToDropOf = (droppedMorph, hand) => {
    if (droppedMorph instanceof DialogBoxMorph) {
      ...
      if (droppedMorph instanceof CommentMorph) {
        this.currentSprite.recordUserEdit("scripts", "comment", "delete");
      }
    }
  };
...
IDE_Morph.prototype.createOldSpriteBar = function () {
  ...
  function addRotationStyleButton(rotationStyle) {
    ...
    button.setLeft(
      button.left() + (rotationStyleButtons.length - 1) * (button.height() + 2)
    );
...
IDE_Morph.prototype.createSpriteEditor = function () {
  ...
  if (this.currentTab === "scripts") {
    ...
    if (this.performerMode) {
      ...
      scripts.cachedTexture = MorphicPreferences.isFlat
        ? this.scriptsTexture()
        : this.scriptsTexture();
    }
    ...
    this.spriteEditor = new WardrobeMorph(this.currentSprite, this.sliderColor);
    this.spriteEditor.color = this.groupColor;
    this.add(this.spriteEditor);
    ...
    this.spriteEditor = new JukeboxMorph(this.currentSprite, this.sliderColor);
    this.spriteEditor.color = this.groupColor;
    this.add(this.spriteEditor);
    this.spriteEditor.updateSelection();
...
IDE_Morph.prototype.createCorral = function (keepSceneAlbum) {
  ...
  // scenes corral
  this.corral.album = keepSceneAlbum
    ? album
...
IDE_Morph.prototype.fixLayout = function (situation) {
  ...
  if (situation !== "refreshPalette") {
    ...
    this.categories.bounds.corner.y =
      this.bottom() - this.extensionButton.height();
...
IDE_Morph.prototype.setExtent = function (point) {
  ...
  if (this.isAppMode) {
    ...
    if (this.stageRatio > 1) {
      ...
      minExt = padding.add(this.stage.dimensions.multiplyBy(this.stageRatio));
    }
  }
  if (this.performerMode) {
    ext = point;
...
IDE_Morph.prototype.droppedImage = function (aCanvas, name, embeddedData, src) {
  ...
  this.hasChangedMedia = true;
  this.currentSprite.recordUserEdit("costume", "imported", costume.name);
};
...
IDE_Morph.prototype.droppedSVG = function (anImage, name) {
  ...
  var myself,
    viewBox,
    w = 300,
    h = 150, // setting HTMLImageElement default values
    scale = 1,
    svgNormalized,
  ...
  if (
    svgObj.attributes.getNamedItem("width") &&
  ...
  if (this.stage.dimensions.x < w || this.stage.dimensions.y < h) {
    scale = Math.min(this.stage.dimensions.x / w, this.stage.dimensions.y / h);
    normalizing = true;
    w = w * scale;
    h = h * scale;
  }
...
IDE_Morph.prototype.droppedAudio = function (anAudio, name) {
  ...
  if (anAudio.src.indexOf("data:audio") !== 0) {
    ...
    this.getURL(
      ...
      (blob) => {
        ...
        reader.onloadend = () => {
          ...
          base64 = "data:audio/mpeg;base64," + base64.split(",")[1];
          anAudio.src = base64;
    ...
    this.oldSpriteBar.tabBar.tabTo("sounds");
    this.hasChangedMedia = true;
    this.currentSprite.recordUserEdit("sound", "imported", name);
  }
};
...
IDE_Morph.prototype.topVisibleCategoryInPalette = function () {
  ...
  top = this.palette.contents.children.find(
    (morph) => morph.category && morph.bounds.intersects(this.palette.bounds)
  );
  if (top) {
...
IDE_Morph.prototype.startFastTracking = function () {
  ...
  //this.projectControlBar.startButton.alpha = 0
  this.projectControlBar.startButton.rerender();
};
...
IDE_Morph.prototype.applySavedSettings = function () {
  ...
  this.getSetting("highContrast")
    ? SpriteMorph.prototype.setHighContrastBlockColor()
    : SpriteMorph.prototype.setDefaultBlockColor();
...
IDE_Morph.prototype.restore = function () {
  ...
  if (this.hasLocalStorage()) {
    if (localStorage["-snap-bakuser-"] == username) {
      ...
      if (bak) {
        this.backup(() => {
          this.openProjectString(bak, () => this.recordUnsavedChanges());
        });
      }
    }
  }
...
IDE_Morph.prototype.newCamSprite = function () {
  ...
  camDialog = new CamSnapshotDialogMorph(
    ...
    () => this.removeSprite(sprite),
    function (costume) {
      // needs to be "function" so it can access "this"
...
IDE_Morph.prototype.makeSureRecordingIsMono = function (sound) {
  ...
  function makeMono(sound) {
    ...
    if (sound.audioBuffer.numberOfChannels === 1) {
      return;
    }
    ...
    audio = new Audio();
    blob = new Blob(
      [audioBufferToWav(encodeSound(samples, 44100).audioBuffer)],
      { type: "audio/wav" }
    );
  ...
  function encodeSound(samples, rate) {
    ...
    arrayBuffer.copyToChannel(Float32Array.from(samples), 0, 0);
...
IDE_Morph.prototype.duplicateSprite = function (sprite) {
  ...
  duplicate.recordUserEdit("corral", "duplicate", sprite.name);
};
...
IDE_Morph.prototype.newSoundName = function (name) {
  ...
  return this.newName(
    name || lastSound.name,
    this.currentSprite.sounds.asArray().map((eachSound) => eachSound.name)
  );
};

IDE_Morph.prototype.newSpriteName = function (name, ignoredSprite) {
  var all = this.sprites
    .asArray()
    .concat(this.stage)
    .filter((each) => each !== ignoredSprite)
    .map((each) => each.name);
...
IDE_Morph.prototype.spriteNamed = function (name) {
  ...
  match = detect(this.sprites, (sprite) => sprite.name === name);
  if (!match) {
...
IDE_Morph.prototype.cloudMenu = function () {
  ...
  if (this.hasCloudProject()) {
    ...
    menu.addItem("Open in Community Site", () => {
      ...
      window.open(
        this.cloud.showProjectPath(dict.Username, dict.ProjectName),
        "_blank"
      );
    });
  }
...
IDE_Morph.prototype.importMedia = function (folderName) {
  ...
  this.getMediaList(folderName, (items) => {
    msg.destroy();
...
IDE_Morph.prototype.aboutSnap = function () {
  ...
  if (versions !== "") {
    versions =
      localize("current module versions:") +
      " \n\n" +
      "morphic (" +
      morphicVersion +
      ")" +
...
IDE_Morph.prototype.scenesMenu = function () {
  ...
  this.scenes
    .asArray()
    .forEach((scn) =>
      menu.addItem([this.scene === scn ? tick : empty, scn.name], scn)
    );
  menu.popup(world, pos);
};
...
IDE_Morph.prototype.deleteUserCategory = function (pos) {
  var menu = new MenuMorph(this.deletePaletteCategory, null, this);
...
IDE_Morph.prototype.deletePaletteCategory = function (name) {
  this.stage.globalBlocks.forEach((def) => {
    if (def.category === name) {
      def.category = "other";
      this.currentSprite
        .allBlockInstances(def)
        .reverse()
        .forEach((block) => block.refresh());
    }
  });
  this.sprites
    .asArray()
    ...
    .forEach((obj) => {
      obj.customBlocks.forEach((def) => {
        if (def.category === name) {
          def.category = "other";
          obj
            .allDependentInvocationsOf(def.blockSpec())
            .reverse()
            .forEach((block) => block.refresh(def));
        }
      });
    });
...
IDE_Morph.prototype.exportProject = function (name) {
  ...
  if (name) {
    ...
    try {
      ...
      str = this.serializer.serialize(new Project(this.scenes, this.scene));
...
IDE_Morph.prototype.exportGlobalBlocks = function () {
  ...
  if (blocks.length > 0) {
    new BlockExportDialogMorph(this.serializer, blocks, this).popUp(
      this.world()
    );
  } else {
    this.inform(
...
IDE_Morph.prototype.removeUnusedBlocks = function () {
  ...
  function scan() {
    return globalBlocks.filter((def) => {
      if (contains(unused, def)) {
        return false;
      }
      return targets.every(
        (each, trgIdx) => !each.usesBlockInstance(def, true, trgIdx, unused)
      );
    });
  }
  ...
  if (unused.length > 0) {
    new BlockRemovalDialogMorph(unused, this).popUp(this.world());
  } else {
    this.inform(
...
IDE_Morph.prototype.generatePuzzle = function () {
  ...
  var current = this.currentSprite,
    ...
    used = current.scripts.allChildren().filter((m) => m instanceof BlockMorph),
    uPrim = [],
    uCust = [],
    uVars = [],
    unused,
  ...
  used.forEach((b) => {
    if (b.isCustomBlock) {
      uCust.push(b.isGlobal ? b.definition : current.getMethod(b.semanticSpec));
  ...
  unused = allBlocks.filter((b) => {
    if (b.isCustomBlock) {
      return !contains(
        uCust,
        b.isGlobal ? b.definition : current.getMethod(b.semanticSpec)
      );
    } else if (b.selector === "reportGetVar") {
  ...
  current.recordUserEdit("palette", "hide block");
...
IDE_Morph.prototype.exportScriptsPicture = function () {
  ...
  this.sprites.asArray().forEach((sprite) => {
    ...
    sprite.customBlocks.forEach((def) => pics.push(def.scriptsPicture()));
  });
  pics.push(this.stage.getImage());
  pics.push(this.stage.scripts.scriptsPicture());
  this.stage.customBlocks.forEach((def) => pics.push(def.scriptsPicture()));

  // collect global block pics
  this.stage.globalBlocks.forEach((def) => pics.push(def.scriptsPicture()));
...
IDE_Morph.prototype.exportProjectSummary = function (useDropShadows) {
      ...
      SpriteMorph.prototype.allCategories().forEach((category) => {
        ...
        definitions.forEach((def) => {
          ...
          if (def.category === category) {
            ...
            li = addNode("li", ul);
            blockImg = addImage(def.templateInstance().scriptPic(), li);
  ...
  if (location.hash.indexOf("#present:") === 0) {
    add(location.toString(), "a", body).attributes.href = location.toString();
    addImage(stage.thumbnail(stage.dimensions)).attributes.class = "sprite";
  ...
  this.sprites
    ...
    .forEach((sprite) => {
      ...
      pic = addImage(sprite.thumbnail(sprite.extent().divideBy(stage.scale)));
      ...
      if (sprite instanceof SpriteMorph) {
        ...
        if (sprite.parts.length) {
          ...
          sprite.parts.forEach((part) => {
            ...
            addImage(
              part.thumbnail(new Point(40, 40)),
              li,
              true
            ).attributes.class = "toc";
          });
      ...
      if (cl > 1 || sprite.getCostumeIdx() !== cl) {
        ...
        sprite.costumes.asArray().forEach((costume) => {
          ...
          addImage(
            costume.thumbnail(new Point(40, 40)),
            li,
            true
          ).attributes.class = "toc";
        });
      ...
      if (sprite.sounds.length()) {
        ...
        ol = addNode("ol");
        sprite.sounds.asArray().forEach((sound) => add(sound.name, "li", ol));
      }
  ...
  if (Object.keys(globalVars.vars).length || stage.globalBlocks.length) {
    addNode("hr");
    add(localize("For all Sprites"), "a", addNode("li", toc)).attributes.href =
      "#global";
...
IDE_Morph.prototype.rawOpenScriptString = function (str, silently) {
  ...
  this.showMessage("Imported Script.", 2);
  this.autoLoadExtensions();
};
...
IDE_Morph.prototype.rawOpenScriptsOnlyString = function (str) {
  ...
  scripts.changed();
  this.oldSpriteBar.tabBar.tabTo("scripts");
  this.showMessage("Imported Scripts.", 2);
};
...
IDE_Morph.prototype.autoLoadExtensions = function () {
  ...
  urls.forEach((url) => {
    ...
    if (
      Process.prototype.enableJS ||
      SnapExtensions.urls.some((any) => url.indexOf(any) === 0)
    ) {
...
IDE_Morph.prototype.saveFileAs = function (contents, fileType, fileName) {
...
IDE_Morph.prototype.toggleZebraColoring = function () {
  ...
  scripts.forEach((topBlock) => topBlock.fixBlockColor(null, true));
};
...
IDE_Morph.prototype.toggleAppMode = function (appMode) {
  ...
  if (this.isAppMode) {
    ...
    this.projectControlBar.appModeButton.refresh();
...
IDE_Morph.prototype.toggleStageSize = function (isSmall, forcedRatio) {
    ...
    world.animations.push(
      new Animation(
        ...
        () => {
          ...
          myself.projectControlBar.stageSizeButton.refresh();
        }
      )
    );
  }
  ...
  if (shiftClicked) {
    smallRatio =
      (SpriteIconMorph.prototype.thumbSize.x * 3) / this.stage.dimensions.x;
    if (!this.isSmallStage || smallRatio === this.stageRatio) {
      toggle();
    }
  } else if (altClicked) {
    smallRatio = this.width() / 2 / this.stage.dimensions.x;
    if (!this.isSmallStage || smallRatio === this.stageRatio) {
      toggle();
    }
  } else {
...
IDE_Morph.prototype.microphoneMenu = function () {
  ...
  if (microphone.isReady) {
    menu.addItem([on, localize("Microphone")], () => microphone.stop());
    menu.addLine();
  }
  resolutions.forEach((res, i) => {
    menu.addItem(
      [microphone.resolution === i + 1 ? tick : empty, localize(res)],
  ...
  menu.addPreference(
    ...
    () => {
      if (SpriteMorph.prototype.isHighContrast) {
        SpriteMorph.prototype.setDefaultBlockColor();
        this.refreshIDE();
        this.removeSetting("highContrast");
        return;
      }
      SpriteMorph.prototype.setHighContrastBlockColor();
      this.saveSetting("highContrast");
      this.refreshIDE();
    },
...
IDE_Morph.prototype.userSetBlocksScale = function () {
  ...
  if (!MorphicPreferences.isFlat && SyntaxElementMorph.prototype.alpha > 0.8) {
  ...
  dlg = new DialogBoxMorph(null, (num) =>
    this.setBlocksScale(Math.min(num, 12))
  ).withKey("zoomBlocks");
...
IDE_Morph.prototype.userSetBlocksAfterglow = function () {
  ...
  action = (num) => {
    ...
    timeout = setTimeout(() => scrpt.removeHighlight(), num * 16, 67);
  };

  dlg = new DialogBoxMorph(null, (num) =>
    this.setBlocksAfterglow(Math.max(0, Math.min(num, 20)))
  ).withKey("afterglow");
  if (MorphicPreferences.isTouchDevice) {
    dlg.isDraggable = false;
  }
...
IDE_Morph.prototype.userSetStageSize = function () {
  new DialogBoxMorph(this, this.setStageExtent, this).promptVector(
    "Stage size",
...
IDE_Morph.prototype.setStageExtent = function (aPoint) {
  ...
  function zoom() {
    myself.step = function () {
      var delta = ext.subtract(myself.stage.dimensions).divideBy(2);
      if (delta.abs().lt(new Point(5, 5))) {
        myself.stage.dimensions = ext;
        delete myself.step;
      } else {
        myself.stage.dimensions = myself.stage.dimensions.add(delta);
      }
...
IDE_Morph.prototype.userSetDragThreshold = function () {
  new DialogBoxMorph(
    this,
    (num) =>
      (MorphicPreferences.grabThreshold = Math.min(Math.max(+num, 0), 200)),
    this
  ).prompt(
    "Dragging threshold",
...
IDE_Morph.prototype.resendVerification = function () {
  ...
  new DialogBoxMorph(null, (user) =>
    ...
    .withKey("cloudresendverification")
    .promptCredentials(
...
IDE_Morph.prototype.hasCloudProject = function () {
  ...
  return (
    params.hasOwnProperty("Username") && params.hasOwnProperty("ProjectName")
  );
};
...
IDE_Morph.prototype.getURL = function (url, callback, responseType) {
  ...
  rsp = !async || request.responseType === "text" ? "responseText" : "response";
  try {
    ...
    if (async) {
      request.onreadystatechange = () => {
        if (request.readyState === 4) {
          if (request[rsp]) {
            callback.call(this, request[rsp]);
          } else {
...
IDE_Morph.prototype.blocksLibraryXML = function (
  ...
  var prims = definitions.filter((def) => def.isGlobal && def.selector),
    ...
    dtaStr =
      dataFrame && dataFrame.names(true).length
        ? this.serializer.serialize(dataFrame, true)
        : "",
    ldtStr =
      localData && localData.names(true).length
...
IDE_Morph.prototype.paletteXML = function (categoryNames) {
  ...
  categoryNames.forEach((cat) => {
    if (SpriteMorph.prototype.customCategories.has(cat)) {
      palette.set(cat, SpriteMorph.prototype.customCategories.get(cat));
    }
  });
...
IDE_Morph.prototype.inform = function (title, message) {
  return new DialogBoxMorph().inform(title, localize(message), this.world());
};
...
IDE_Morph.prototype.prompt = function (message, callback, choices, key) {
  new DialogBoxMorph(null, callback)
    .withKey(key)
    .prompt(message, "", this.world(), null, choices);
};
...
IDE_Morph.prototype.warnAboutDev = function () {
  ...
  this.inform(
    ...
    "This version of Snap! is being developed.\n" +
      ...
      "visit https://snap.berkeley.edu/snap/snap.html\n" +
...
ProjectDialogMorph.prototype.buildContents = function () {
  ...
  if (this.task === "save") {
    thumbnail = this.ide.scenes
      .at(1)
      .stage.thumbnail(SnapSerializer.prototype.thumbnailSize);
...
ProjectDialogMorph.prototype.popUp = function (wrrld) {
  ...
  if (world) {
    ...
    this.handle = new HandleMorph(this, 350, 330, this.corner, this.corner);
  }
};
...
ProjectDialogMorph.prototype.createButtons = function () {
  ...
  this.buttons.fixLayout = function () {
    if (
      this.topRow.children.some(function (any) {
...
ProjectDialogMorph.prototype.buildFilterField = function () {
  ...
  this.filterField.reactToInput = function (evt) {
    ...
    myself.listField.elements = myself.projectList.filter((aProject) => {
...
ProjectDialogMorph.prototype.hasLocalProjects = function () {
  ...
  return Object.keys(localStorage).some(
    (any) => any.indexOf("-snap-project-") === 0
  );
};
...
ProjectDialogMorph.prototype.getLocalProjectList = function () {
  ...
  for (stored in localStorage) {
    if (
      Object.prototype.hasOwnProperty.call(localStorage, stored) &&
  ...
  if (name) {
    if (this.source === "cloud") {
      if (detect(this.projectList, (item) => item.projectname === name)) {
        this.ide.confirm(
          localize("Are you sure you want to replace") + '\n"' + name + '"?',
          "Replace Project",
          () => {
...
ProjectDialogMorph.prototype.shareProject = function () {
  ...
  if (proj) {
    this.ide.confirm(
      localize("Are you sure you want to share") +
        '\n"' +
        proj.projectname +
        '"?',
      "Share Project",
...
ProjectDialogMorph.prototype.unshareProject = function () {
  ...
  if (proj) {
    this.ide.confirm(
      localize("Are you sure you want to unshare") +
        '\n"' +
        proj.projectname +
        '"?',
      "Unshare Project",
...
ProjectDialogMorph.prototype.publishProject = function () {
  ...
  if (proj) {
    this.ide.confirm(
      localize("Are you sure you want to publish") +
        '\n"' +
        proj.projectname +
        '"?',
      "Publish Project",
...
ProjectRecoveryDialogMorph.prototype.buildListField = function () {
  ...
  this.listField.action = (item) => {
    ...
    version = detect(this.versions, (version) => version.lastupdated === item);
    ...
    (versions) => {
      ...
      this.versions.forEach((version) => {
        ...
        if (date.toDateString() === today.toDateString()) {
          ...
          version.lastupdated =
            localize("Yesterday, ") + date.toLocaleTimeString();
        } else {
          version.lastupdated = date.toLocaleString();
        }
      });
      this.listField.elements = this.versions.map(
        (version) => version.lastupdated
      );
      this.clearDetails();
...
ProjectRecoveryDialogMorph.prototype.popUp = function () {
  ...
  if (world) {
    ...
    this.handle = new HandleMorph(this, 300, 300, this.corner, this.corner);
  }
};
...
ProjectRecoveryDialogMorph.prototype.fixLayout = function () {
  ...
  if (this.body) {
    this.body.setPosition(
      this.position().add(new Point(this.padding, titleHeight + this.padding))
    );
    this.body.setExtent(
      new Point(
        this.width() - this.padding * 2,
    ...
    this.listField.setWidth(
      this.body.width() - this.preview.width() - this.padding
    );
    this.listField.contents.children[0].adjustWidths();
...
LibraryImportDialogMorph.prototype.buildFilterField = function () {
  ...
  this.filterField.reactToInput = function (evt) {
    ...
    myself.filteredLibrariesList = myself.librariesData.filter(
      (library) => librarySearcText(library).indexOf(text) > -1
    );
...
LibraryImportDialogMorph.prototype.popUp = function () {
  ...
  if (world) {
    ...
    this.handle = new HandleMorph(this, 500, 500, this.corner, this.corner);
  }
};
...
LibraryImportDialogMorph.prototype.fixLayout = function () {
  ...
  if (this.body) {
    ...
    this.listField.setTop(inputField.bottom() + this.padding);
    this.listField.setHeight(this.body.height() - inputField.height());
    this.listField.contents.children[0].adjustWidths();

    this.notesField.setExtent(
      new Point(this.body.width() - this.listField.width() - thin, 100)
    );
    ...
    this.palette.setPosition(this.listField.topRight().add(new Point(thin, 0)));
...
LibraryImportDialogMorph.prototype.importLibrary = function () {
  ...
  ide.showMessage(`${localize("Loading")} ${libraryName}`);
  ide.getURL(ide.resourceURL("libraries", selectedLibrary), (libraryText) => {
    ide.droppedText(libraryText, libraryName);
    this.isLoadingLibrary = true;
  });
};
...
SpriteIconMorph.prototype.fixLayout = function () {
  ...
  this.bounds.setWidth(
    this.thumbnail.width() + this.outline * 2 + this.edge * 2 + this.padding * 2
  );
  ...
  this.thumbnail.setTop(this.top() + this.outline + this.edge + this.padding);
...
SpriteIconMorph.prototype.userMenu = function () {
  ...
  if (this.object instanceof StageMorph) {
    menu.addItem(
      ...
      () => {
        var ide = this.parentThatIsA(IDE_Morph);
        ide.saveCanvasAs(this.object.fullImage(), this.object.name);
      },
      "save a picture\nof the stage"
    );
  ...
  menu.addItem("rename", () => {
    new DialogBoxMorph(null, (x) => {
      this.object.setName(
        this.parentThatIsA(IDE_Morph).newSpriteName(x, this.object)
      );
      this.step();
      this.fixLayout();
      //this.parentThatIsA(IDE_Morph).oldSpriteBar.nameField.setContents(this.object.name)
    }).prompt("Rename Sprite", "", world);
  });
  ...
  if (this.object.solution) {
    menu.addItem("extract solution", () => {
      this.parentThatIsA(IDE_Morph).undelete(
        this.object.solution.fullCopy(),
        this.center()
      );
    });
    menu.addItem("delete solution", () => {
      this.parentThatIsA(IDE_Morph).removeSprite(this.object.solution);
      this.object.solution = null;
      this.object.recordUserEdit("sprite", "solution", "delete");
    });
    menu.addLine();
  }
  if (StageMorph.prototype.enableInheritance) {
  ...
  if (this.object.anchor) {
    menu.addItem(localize("detach from") + " " + this.object.anchor.name, () =>
      this.object.detachFromAnchor()
    );
  }
  if (this.object.parts.length) {
    menu.addItem("detach all parts", () => this.object.detachAllParts());
  }
...
SpriteIconMorph.prototype.releaseSprite = function () {
  this.object.release();
  this.object.recordUserEdit("sprite", "release", this.object.name);
};
...
SpriteIconMorph.prototype.flash = function () {
  ...
  world.animations.push(
    new Animation(nop, nop, 0, 800, nop, () => this.flashOff())
  );
};
...
CostumeIconMorph.prototype.createThumbnail = function () {
  ...
  watermark =
    this.object instanceof SVG_Costume
      ? "svg"
...
CostumeIconMorph.prototype.step = SpriteIconMorph.prototype.step;

// CostumeIconMorph layout

CostumeIconMorph.prototype.fixLayout = SpriteIconMorph.prototype.fixLayout;
...
CostumeIconMorph.prototype.renameCostume = function () {
  ...
  new DialogBoxMorph(null, (answer) => {
    var old = costume.name;
    if (answer && answer !== costume.name) {
      costume.name = wardrobe.sprite.newCostumeName(answer, costume);
      costume.version = Date.now();
      ide.hasChangedMedia = true;
      wardrobe.sprite.recordUserEdit("costume", "rename", old, costume.name);
    }
  }).prompt(
...
CostumeIconMorph.prototype.typeOfStringData = function (aString) {
  ...
  if (aString[0] === "<") {
    if (
      ["project", "snapdata", "blocks", "sprites", "block", "script"].some(
        (tag) => aString.slice(1).startsWith(tag)
      )
    ) {
      return "blocks";
    }
...
CostumeIconMorph.prototype.render = SpriteIconMorph.prototype.render;
...
TurtleIconMorph.prototype.createLabel = function () {
  ...
  txt = new StringMorph(
    localize(this.object instanceof SpriteMorph ? "Turtle" : "Empty"),
    this.fontSize,
    this.fontStyle,
    true,
    false,
...
TurtleIconMorph.prototype.fixLayout = SpriteIconMorph.prototype.fixLayout;
...
WardrobeMorph.prototype.newFromCam = function () {
  ...
  camDialog = new CamSnapshotDialogMorph(ide, sprite, nop, (costume) => {
    sprite.addCostume(costume);
    sprite.wearCostume(costume);
    this.updateList();
    sprite.recordUserEdit("costume", "snap", costume.name);
  });
...
WardrobeMorph.prototype.reactToDropOf = function (icon) {
  ...
  this.sprite.recordUserEdit("costume", "add", costume.name, idx + 1);
};
...
SoundIconMorph.prototype.fixLayout = SpriteIconMorph.prototype.fixLayout;
...
SoundIconMorph.prototype.renameSound = function () {
  ...
  new DialogBoxMorph(null, (answer) => {
    ...
    if (answer && answer !== old) {
      ...
      ide.hasChangedMedia = true;
      ide.currentSprite.recordUserEdit("sound", "rename", old, sound.name);
    }
...
SoundIconMorph.prototype.render = SpriteIconMorph.prototype.render;
...
JukeboxMorph.prototype.reactToDropOf = function (icon) {
  ...
  this.sprite.recordUserEdit("sound", "add", sound.name, idx + 1);
};
...
SceneIconMorph.prototype.fixLayout = SpriteIconMorph.prototype.fixLayout;
...
SceneIconMorph.prototype.renameScene = function () {
  ...
  new DialogBoxMorph(null, (answer) => {
    if (answer && answer !== scene.name) {
      scene.name = ide.newSceneName(answer, scene);
  ...
  }).prompt("rename scene", scene.name, this.world());
};
...
SceneIconMorph.prototype.render = SpriteIconMorph.prototype.render;
...
StageHandleMorph.prototype.renderOn = function (ctx, color, shadowColor) {
  var l = this.height() / 8,
    w = this.width() / 6,
    r = w / 2,
    x,
...
StageHandleMorph.prototype.mouseDownLeft = function (pos) {
  ...
  ide.projectControlBar.stageSizeButton.refresh();
  this.lockMouseFocus();
};
...
PaletteHandleMorph.prototype.render = StageHandleMorph.prototype.render;

PaletteHandleMorph.prototype.renderOn = StageHandleMorph.prototype.renderOn;
...
PaletteHandleMorph.prototype.mouseEnter = StageHandleMorph.prototype.mouseEnter;

PaletteHandleMorph.prototype.mouseLeave = StageHandleMorph.prototype.mouseLeave;
...
SoundRecorderDialogMorph.prototype.buildProgressBar = function () {
  ...
  this.progressBar.step = function () {
    if (myself.audioElement.duration) {
      this.setPercentage(
        (myself.audioElement.currentTime / myself.audioElement.duration) * 100
      );
    } else {
  ...
  if (this.audioElement.readyState === 4) {
    ...
    this.audioElement.oncanplaythrough = function () {
      if (this.duration && this.duration !== Infinity) {
        ...
        myself.buttons.children.forEach((button) => button.disable());
        this.play();
      }
    };
  }
